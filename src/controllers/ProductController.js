...
class ProductController {
  getProducts = async (req, res) => {
		try {
			...
			} else { return res.sendUserError(404, { error: 'Not found' }); }
		} catch (error) {
	...
	createProduct = async (req, res) => {
		try {
			...
			return res.sendSuccess(201, { response: `product "${newProduct._id}" created` });
		} catch (error) {
			res.sendServerError(error);
		}
	}
	...
	updateProduct = async (req, res) => {
		try {
			...
			if (Object.entries(productData).length !== 0) {
				...
				else { return res.sendUserError(404, { response: 'Product not found' });	}
			} else {
